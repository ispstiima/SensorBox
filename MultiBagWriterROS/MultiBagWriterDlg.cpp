...
BOOL CMultiBagWriterDlg::OnInitDialog()
{
	...
	m_checkRotateImages.SetCheck(BST_CHECKED);
	Controller.bImageRotation = true;
...
void CMultiBagWriterDlg::OnBnClickedStartgrab()
{
	...
	FileDlg.m_ofn.lpstrTitle = _T("Select basename for output bags");
	if (FileDlg.DoModal() == IDOK) {
		CString tString(FileDlg.GetPathName());
		int extPos = tString.Find(L".", tString.GetLength() - 4);
		if (extPos >= 0)
			Controller.m_FilePathAndName = tString.Left(extPos);
		else
			Controller.m_FilePathAndName = tString;
...
{
	Controller.bImageRotation = !Controller.bImageRotation;
}
